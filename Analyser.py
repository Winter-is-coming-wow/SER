...
class analyser():
    ...
    def __init__(self, subdir_model=None):
        ...
        # Emotion encoding
        self._emotion = {0: 'neutral', 1: 'calm', 2: 'happy', 3: 'sad', 4: 'angry', 5: 'fearful', 6: 'disgust',
                         7: 'surprised'}
    ...
    def build_model(self):
        ...
        y = TimeDistributed(Conv2D(64, kernel_size=(3, 3), strides=(1, 1), padding='same'), name='Conv_2_MELSPECT')(
            y)
    ...
    def predict(self,y,chunk_size=49100,sample_rate=16000):
        y=np.array(y)
        y = zscore(y)
        ...
        predict_no = np.argmax(self._model.predict(mel_spect_ts), axis=1)
        predict = [self._emotion.get(emotion) for emotion in predict_no]
        print(time.ctime(),'  :  ',predict_no,predict)
